<?php
/**
 * Plugin Name: KO Admin Username Changer
 * Plugin URI:  https://kevinocasio.com/tools/ko-admin-username-changer/
 * Description: Safely change your WordPress username to harden security or update your login handle without database scripts.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ko-admin-username-changer
 */
...
define('KO_AUC_VERSION', '1.0.0');
define('KO_AUC_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('KO_AUC_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ko_auc_brand_url($path = '/tools/') {
...
/**
 * 1. Admin Menu Placement (KO Plugins Suite at Position 2)
 */
function ko_auc_admin_menu()
{
	...
	// Register Dedicated Action Screen Submenu
	add_submenu_page(
		'ko-plugins-main',
...
add_action('admin_menu', 'ko_auc_admin_menu');

/**
 * 2. Action Link on Plugins Screen
 */
function ko_auc_settings_action_link($links)
{
	$settings_url = admin_url('admin.php?page=ko-admin-username-changer');
	$settings_link = '<a href="' . esc_url($settings_url) . '">Change Username</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ko_auc_settings_action_link');

/**
 * 3. Conditional Admin Asset Enqueuing (Strict Anti-Bloat)
 */
function ko_auc_enqueue_admin_assets($hook)
{
	$allowed_hooks = array(
		'toplevel_page_ko-plugins-main',
		'ko-plugins_page_ko-admin-username-changer',
	);
	...
	$ver = file_exists(KO_AUC_PLUGIN_DIR . 'assets/admin.css') ? filemtime(KO_AUC_PLUGIN_DIR . 'assets/admin.css') : KO_AUC_VERSION;
	wp_enqueue_style('ko-auc-admin-css', plugins_url('assets/admin.css', __FILE__), array(), $ver);
}
add_action('admin_enqueue_scripts', 'ko_auc_enqueue_admin_assets');

/**
 * 4. Core Logic: Safe Database Username Mutation & Session Refresh
 */
function ko_auc_handle_submit()
{
	if (!isset($_POST['ko_auc_submit'])) {
		return;
	}

	check_admin_referer('ko_auc_action', 'ko_auc_nonce');

	if (!current_user_can('manage_options')) {
		wp_die(__('You do not have sufficient permissions to perform this action.', 'ko-admin-username-changer'));
	}

	$new_username = isset($_POST['ko_auc_new_username']) ? sanitize_user(trim($_POST['ko_auc_new_username']), true) : '';
	$current_user = wp_get_current_user();
	$old_username = $current_user->user_login;

	// Validation
	if (empty($new_username)) {
		add_settings_error('ko_auc_messages', 'ko_auc_empty', __('Username cannot be empty.', 'ko-admin-username-changer'), 'error');
		return;
	}

	if ($new_username === $old_username) {
		add_settings_error('ko_auc_messages', 'ko_auc_same', __("That's already your current username.", 'ko-admin-username-changer'), 'error');
		return;
	}

	if (!validate_username($new_username)) {
		add_settings_error('ko_auc_messages', 'ko_auc_invalid', __('Username contains invalid characters.', 'ko-admin-username-changer'), 'error');
		return;
	}

	if (username_exists($new_username)) {
		add_settings_error('ko_auc_messages', 'ko_auc_exists', __('This username is already taken. Please choose another.', 'ko-admin-username-changer'), 'error');
		return;
	}

	// Safe Direct Database Update
	global $wpdb;
	$result = $wpdb->update(
		$wpdb->users,
	...
	if ($result !== false) {
		// Refresh auth cookies to prevent session dropouts
		clean_user_cache($current_user->ID);
		wp_clear_auth_cookie();
		wp_set_current_user($current_user->ID);
		wp_set_auth_cookie($current_user->ID);

		// Safe redirect to prevent re-posts and trigger success state
		$redirect_url = add_query_arg(
			array(
				'page'    => 'ko-admin-username-changer',
				'updated' => 'true',
			),
			admin_url('admin.php')
		);
		...
		add_settings_error('ko_auc_messages', 'ko_auc_db_error', __('Database error. Could not update username.', 'ko-admin-username-changer'), 'error');
	}
}
add_action('admin_init', 'ko_auc_handle_submit');

/**
 * 5. Dedicated Action Screen HTML (Centered 620px Enclosed Card)
 */
function ko_auc_render_page()
{
	if (!current_user_can('manage_options')) {
		return;
	}
	...
	$current_user = wp_get_current_user();
	$errors = get_settings_errors('ko_auc_messages');
	$is_updated = isset($_GET['updated']) && 'true' === $_GET['updated'];

	$hub_url = ko_auc_brand_url('/tools/');
	$author_url = ko_auc_brand_url('/');
	?>
	<div class="wrap ko-plugin-wrap">
		<!-- Hidden heading so WordPress outputs third-party notices above the card -->
		<h1 class="wp-heading-inline" style="display:none;"></h1>

		<div class="ko-plugin-card">
			<!-- Header Bar (Zero Status Badges) -->
			<div class="ko-plugin-header">
				<h2 class="ko-plugin-header-title">
					<span class="ko-logo-k">K</span><span class="ko-logo-o">O</span><span class="ko-title-text">ADMIN USERNAME CHANGER</span>
				</h2>
			</div>

			<!-- Content Stage -->
			<div class="ko-plugin-body">
				<p class="ko-plugin-intro">Safely update your login username to harden security or change your administrator handle.</p>

				<?php if (!empty($errors)): ?>
					<?php foreach ($errors as $error): ?>
						<div class="ko-alert-error">
				...
				<form method="post" action="">
					<?php wp_nonce_field('ko_auc_action', 'ko_auc_nonce'); ?>
					...
					<div class="ko-setting-box">
						...
						<div class="ko-setting-row">
							<div class="ko-setting-info">
								...
								<p>Your active login handle in WordPress.</p>
							</div>
							<input type="text" value="<?php echo esc_attr($current_user->user_login); ?>" disabled class="ko-input-text">
						</div>
						...
						<div class="ko-setting-row">
							<div class="ko-setting-info">
								...
								<p>Enter your desired new login handle.</p>
							</div>
							<input type="text" name="ko_auc_new_username" placeholder="Enter new username..." class="ko-input-text" required autocomplete="off">
						</div>
					</div>

					<div class="ko-submit-wrap">
						<?php
						$btn_text = $is_updated ? 'Username Updated!' : 'Update Username';
						$btn_class = $is_updated ? 'ko-btn-submit ko-btn-saved' : 'ko-btn-submit';
						?>
						<button type="submit" name="ko_auc_submit" id="ko-auc-submit-btn" class="<?php echo esc_attr($btn_class); ?>">
							<?php echo esc_html($btn_text); ?>
						</button>
						<p class="ko-submit-note">Note: Changing your username updates your login credentials immediately.</p>
					</div>
				</form>
			</div>
	...
	document.addEventListener('DOMContentLoaded', function() {
		var btn = document.getElementById('ko-auc-submit-btn');
...
/**
 * 6. Master KO Plugins Suite Dashboard Callback (17-Plugin Mission Control Grid)
 */
		...
		<div class="wrap ko-dash-wrap">
			...
			<div class="ko-dash-grid">
				<?php foreach ($suite_plugins as $plugin):
					...
					$plugin_url = ko_auc_brand_url('/tools/' . $plugin['slug'] . '/');
					?>
